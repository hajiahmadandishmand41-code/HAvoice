...
if ($slug !== '') { foreach (admin_load('media') as $m) { if (($m['slug'] ?? '') === $slug && ($m['type'] ?? '') === 'video') { $item = $m; break; } } if (!$item) foreach (media_items() as $m) { if (($m['slug'] ?? '') === $slug && ($m['type'] ?? '') === 'video') { $item = $m; break; } } }
$flash = flash();
?>
