...
if (trim((string) ($_POST['website'] ?? '')) !== '') {
    /* ربات هیچ نشانه‌ای از «رد شدن» نمی‌بیند: همان پیامِ موفقیتِ معمولی. */
    flash('success', 'پیام شما ثبت شد. ممنون! معمولاً تا دو روز کاری پاسخ می‌دهیم.');
    redirect($backUrl);
}
...
/* ۷) ذخیره: اول دیتابیس (منبعِ حقیقتِ پنلِ مدیریت)، بعد فایلِ CSV
      (پشتیبانِ قابلِ بازکردن در اکسل). موفقیتِ هرکدام کافی است. */
$saved = false;

if (db_table_exists('ha_contact_messages')) {
    $saved = db_message_save($record) || $saved;
}
...
if ($saved) {
    flash('success', 'پیام شما ثبت شد و در پنلِ مدیریت دیده می‌شود. ممنون! معمولاً تا دو روز کاری با همان ایمیل پاسخ می‌دهیم.');
} else {
