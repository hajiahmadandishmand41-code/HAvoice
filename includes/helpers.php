<?php
/**
 * HAvoice — توابع کمکی مشترک (هسته‌ی سبک و بدون وابستگی خارجی)
 */
...
/**
 * بارگذاری یک فایل داده از پوشه data/ (یک‌بار برای هر درخواست).
 * فایل‌های داده فقط آرایه‌ی PHP برگردانده‌اند؛ بدون دیتابیس.
 */
...
/** آیتم‌های ناوبری اصلی (سازگار با هر دو حالت آدرس‌دهی). */
...
/** پیشوند فیزیکی مسیر نصب (برای فایل‌های assets در حالت URL کوتاه). */
...
/** نشانی فایل assets: در حالت عادی نسبی (سازگار با نصب در پوشه‌ی فرعی). */
...
/** جدول مسیرها: تنها راه ورود به صفحات — جلوی Local File Inclusion را می‌گیرد. */
function routes(): array
{
    ...
    $table = [
        ...
        'course'    => ['file' => 'course.php',    'pretty' => 'course',    'title' => 'مسیر آموزشی'],
        ...
        'exercises' => ['file' => 'exercises.php', 'pretty' => 'exercises', 'title' => 'تمرین‌های عملی'],
        'tips'      => ['file' => 'tips.php',      'pretty' => 'tips',      'title' => 'نکات کوتاه'],
        'about'     => ['file' => 'about.php',     'pretty' => 'about',     'title' => 'درباره های‌ویس'],
        'contact'   => ['file' => 'contact.php',   'pretty' => 'contact',   'title' => 'تماس با ما', 'session' => true],
        'search'    => ['file' => 'search.php',    'pretty' => 'search',    'title' => 'جستجوی مقالات'],
    ];
...
/** برچسب/نامک: فقط حروف، رقم، خط تیره. */
...
/** برچسب زمان مطالعه/تمرین به فارسی. */
...
function csrf_verify(): bool
{
    ...
    return $sent !== '' && session_status() === PHP_SESSION_ACTIVE
        && !empty($_SESSION['ha_csrf'])
        && hash_equals($_SESSION['ha_csrf'], $sent);
}
...
/**
 * متن‌های کوتاه داخل داده‌ها از پیش امن‌سازی می‌شوند؛ این تابع
 * ابتدا escape و سپس تبدیل‌های ساده‌ی درون‌خطی را اعمال می‌کند.
 * پشتیبانی از: **مهم** و *تأکید* و `کد`.
 */
...
/** رندر بلوک‌های محتوایی که در فایل‌های data/ تعریف شده‌اند. */
    ...
    foreach ($blocks as $block) {
        ...
        switch ($block['type']) {
            ...
            case 'h3':
                ...
                $heading++;
                $html[] = '<h' . $level . ' id="sec-' . $heading . '">'
                    . inline((string) ($block['text'] ?? '')) . '</h' . $level . '>';
                break;
            ...
            case 'quote':
                $html[] = '<blockquote class="quote">' . inline((string) ($block['text'] ?? ''))
                    . (empty($block['by']) ? '' : '<cite>' . e($block['by']) . '</cite>')
                    . '</blockquote>';
                break;

            case 'tip':
                $html[] = '<aside class="callout callout--' . e($block['tone'] ?? 'tip') . '">'
                    . '<span class="callout__icon" aria-hidden="true">' . e(icon_name($block['tone'] ?? 'tip')) . '</span>'
                    . '<div><h3>' . e($block['title'] ?? 'نکته') . '</h3><p>' . inline((string) ($block['text'] ?? '')) . '</p></div>'
                    . '</aside>';
                break;
...
/** جعبه‌ی «تمرین همین درس» که داخل مسیر آموزشی نمایش داده می‌شود. */
function render_drill(array $block): string
{
    $items = '';
    foreach ((array) ($block['items'] ?? []) as $item) {
        $items .= '<li><span class="tick" aria-hidden="true">✓</span><span>' . inline((string) $item) . '</span></li>';
    }

    return '<section class="drill">'
        . '<header class="drill__head"><h3>' . e($block['title'] ?? 'تمرین') . '</h3>'
        . (empty($block['time']) ? '' : '<span class="chip chip--ghost">' . minutes_label((int) $block['time']) . '</span>')
        . '</header><ul class="drill__list">' . $items . '</ul>'
        . (empty($block['note']) ? '' : '<p class="drill__note">' . inline((string) $block['note']) . '</p>')
        . '</section>';
}

/** نماد متنی بلوک‌های نکته (بدون کتابخانه آیکون). */
...
/**
 * جستجوی فارسی‌پسند: نرمال‌سازی نیم‌فاصله/ي عربی، حذف ویرگول‌های اضافی
 * و امتیازدهی (عنوان وزن بیشتری دارد).
 */
function search_articles(string $query, array $articles, int $limit = 0): array
{
    $needle = normalize_persian($query);
    $words  = array_values(array_filter(preg_split('/\s+/', $needle) ?: [], static function ($w) {
    ...
    foreach ($articles as $index => $article) {
        ...
        foreach ($words as $word) {
            ...
            if (mb_strpos($excerpt, $word, 0, 'UTF-8') !== false) {
                $score += 6;
                $hit = true;
            }
            if (mb_strpos($haystack, $word, 0, 'UTF-8') !== false) {
                $score += 2;
                $hit = true;
            }
            if ($hit) {
        ...
        if ($matched === count($words)) {
            $score += 5; // همه کلمات پیدا شده‌اند
        }
...
function normalize_persian(string $text): string
{
    ...
    $map   = [
        ...
        'ِ'         => '',
        'ُ'         => '',
        "\u{00A0}" => ' ', // فاصله‌ی ناشکن
        "\u{200f}" => '',  // نشان جهت راست‌به‌چپ
    ];
    $text  = strtr($text, $map);
...
/** خلاصه‌ی امن هر مقاله (برش نرم به تعداد نویسه). */
function excerpt_of(array $article, int $limit = 150): string
{
    ...
    return e(mb_strlen($text, 'UTF-8') > $limit
        ? rtrim(mb_substr($text, 0, $limit, 'UTF-8')) . '…'
        : $text);
}

/** پیدا کردن یک آیتم بر اساس نامک، با برگرداندن جای‌نما. */
function find_by_slug(array $items, string $slug): array
{
    $slug = slugify($slug);

    foreach ($items as $index => $item) {
        if (slugify((string) ($item['slug'] ?? '')) === $slug) {
...
function active_route(): string
{
    return $GLOBALS['HA_ROUTE'] ?? 'home';
}
...
function is_current(string $route): bool
{
    ...
    return ($route === 'articles' && $current === 'article')
        || ($route === 'course' && $current === 'lesson');
}

/** شماره‌ی ترتیب آیتم برای نمایش «گام ۱ از ۹». */
function fa_ordinal(int $n, int $total = 0): string
{
    return $total > 0
        ? fa_num($n) . ' از ' . fa_num($total)
        : fa_num($n);
}
...
/* ------------------------------------------------------------------ */
/*  پیام‌های آنی (flash) و ورودی قبلی فرم                              */
/* ------------------------------------------------------------------ */
...
function flash(string $type = '', string $message = ''): array
{
    ...
    if ($type !== '') {
        $_SESSION['ha_flash'] = ['type' => $type, 'message' => $message];

        return [];
    }

    $flash = $_SESSION['ha_flash'] ?? null;
    unset($_SESSION['ha_flash']);
...
{
    return isset($_SESSION['ha_old'][$key]) ? (string) $_SESSION['ha_old'][$key] : $default;
}

function old_set(array $values): void
{
    if (session_status() === PHP_SESSION_ACTIVE) {
        $_SESSION['ha_old'] = $values;
    }
}
...
{
    if (session_status() === PHP_SESSION_ACTIVE) {
        unset($_SESSION['ha_old']);
    }
}

/** برچسب حالت فرم برای هر فیلد (کلاس خطا). */
